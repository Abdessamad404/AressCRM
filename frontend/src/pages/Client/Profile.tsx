  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {fields.length === 0 && (
          <p className="text-xs text-gray-400">Click "Add" to add items</p>
        )}
      </div>
    </div>
  );
...
export default function CommercialProfile() {
  ...
  const { register, control, handleSubmit, reset, watch, formState: { errors, isSubmitting, isDirty } } = useForm<FormData>({
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => profileApi.upsertProfile({
      ...data,
      skills:       (data.skills ?? []).map((s) => s.value).filter(Boolean),
      expertise:    (data.expertise ?? []).map((e) => e.value).filter(Boolean),
      sectors:      (data.sectors ?? []).map((s) => s.value).filter(Boolean),
      achievements: (data.achievements ?? []).map((a) => a.value).filter(Boolean),
      experience_years: data.experience_years === '' ? undefined : data.experience_years as number,
      commission_rate:  data.commission_rate === '' ? undefined : data.commission_rate as number,
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {profile ? 'Update your commercial profile' : 'Create your commercial profile to get discovered by entreprises'}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('linkedin_url')} placeholder="https://linkedin.com/in/..." className={inputClass} />
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 space-y-3">
          ...
          {achFields.map((field, i) => (
            <div key={field.id} className="flex gap-2">
              ...
              <button type="button" onClick={() => removeAch(i)} className="p-2 text-red-400 hover:text-red-600 transition-colors">
                <Trash2 size={14} />
              </button>
            </div>
          ))}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Make your profile visible to entreprises searching for talent
              </p>
            </div>
